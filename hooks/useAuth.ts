...
import { doc, onSnapshot, setDoc, getDoc, query, collection, where, getDocs } from 'firebase/firestore';
...
export const useAuth = () => {
  ...
  const createOrganization = useCallback(async (name: string) => {
    ...
    try {
      ...
      setOrgId(newOrgId);
      toast.success('মাদরাসা সফলভাবে তৈরি করা হয়েছে!');
    } catch (error) {
      console.error("Error creating organization:", error);
      toast.error('মাদরাসা তৈরি করতে সমস্যা হয়েছে।');
    }
  }, [user]);
  ...
  const joinOrganization = useCallback(async (identifier: string) => {
    ...
    try {
      ...
      toast.success('মাদরাসায় যোগদান সফল হয়েছে!');
    } catch (error: any) {
      console.error("Error joining organization:", error);
      toast.error(error.message || 'মাদরাসায় যোগদান করতে সমস্যা হয়েছে।');
    }
  }, [user]);
  ...
  const leaveOrganization = useCallback(async () => {
    ...
    try {
      ...
      setOrgId(null);
      toast.success('মাদরাসা ত্যাগ করা হয়েছে।');
    } catch (error) {
      console.error("Error leaving organization:", error);
      toast.error('মাদরাসা ত্যাগ করতে সমস্যা হয়েছে।');
    }
  }, [user]);
